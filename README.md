...
**It names the whole module graph up front.** `index.html` gets a `<link rel="modulepreload">` for every module reachable from `app.js`. Without them the browser discovers the graph in waves — the page finds `app.js`, which finds `rfb.js`, which finds twenty more — and each wave costs a round trip before the next can start. The list is generated by following the imports, never written by hand.
